...
export class ServiceNotasEtapasService {
  ...
  getEtapaParticipante(id:number): Observable<EtapaParticipante[]> {
    ...
    return this.http.get<EtapaParticipante[]>(url, {headers: new HttpHeaders({"Authorization": "Bearer " + this.token,
  ...
  getEtapas(): Observable<Etapas[]>{
    ...
    return this.http.get<Etapas[]>(url,{headers: new HttpHeaders({"Authorization": "Bearer " + this.token,
    "Content-Type": "application/json"})}).pipe(
